...
# svg_path = project_path + '/iconos/interruptor.svg'  # Definir la ruta del archivo SVG para las cargas
# svg_marker = QgsSvgMarkerSymbolLayer(svg_path) # Crear una nueva instancia de QgsSvgMarkerSymbolLayer para las cargas
# svg_marker.setSize(5)  # Establecer el tamaño del marcador SVG para las cargas
# svg_marker.setOffset(QPointF(0, 0))  # Establecer el desplazamiento del marcador SVG para las cargas
# symbol = QgsSymbol.defaultSymbol(load_layer.geometryType())    # Crear un nuevo símbolo y añadir la capa de marcador SVG para las cargas
# symbol.changeSymbolLayer(0, svg_marker)
# switch_layer.renderer().setSymbol(symbol)    # Establecer el símbolo para la capa de cargas
# switch_layer.triggerRepaint()    # Actualizar la capa de cargas para aplicar el nuevo estilo

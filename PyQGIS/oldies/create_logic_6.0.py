...
class CustomToolbarPanel:
    ...
    def handle_button_click(self, button_name, cursor_icon_path):
        ...
        if button_name.lower() == "seleccion":
            ...
            estado_edicion = "Selec_None"
            iface.mapCanvas().unsetCursor()
            import_dss(self.map_tool.node_mt_layer, self.map_tool.line_mt_layer, self.map_tool.transformer_layer)
...
def import_dss(node_mt_layer, line_layer, transformer_layer):
    ...
    total_elements = node_count + line_count + transformer_count
    ...
    transformer_idx = dss_instance.dssTransformers.First  # Iniciar con el primer transformador
    while transformer_idx:  # Mientras haya un transformador activo
        transformer_name = dss_instance.dssTransformers.Name  # Obtener el nombre del transformador actual
        ...
        #print(f"Nodos extremos del transformador {transformer_name}: {barra1} - {barra2}")  # Imprimir los nodos extremos

        dss_instance.activar_barra(barra1)
        x, y = dss_instance.dssBus.x, dss_instance.dssBus.y

        if x != 0 and y != 0:
            transformer_feature = QgsFeature(transformer_layer.fields())
            transformer_feature.setGeometry(QgsGeometry.fromPointXY(QgsPointXY(x, y)))
            transformer_feature.setAttribute("id", transformer_name)
            transformer_feature.setAttribute("wdg1_node", barra1)
            transformer_feature.setAttribute("wdg2_node", barra2)

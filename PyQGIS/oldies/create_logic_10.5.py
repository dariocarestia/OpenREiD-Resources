    ...
    def canvasPressEvent(self, event): #Este evento de dispara al presionar, no al hacer clic (presionar y soltar)
        ...
        if estado_nivel_tension == "MT":
            ...
            if event.button() == Qt.RightButton:
                ...
                elif estado_flujo_trabajo in ["carga", "sistema_fotovoltaico", "capacitor", "generador"]:
                    ...
                    if estado_seleccion_linea == "Selec_None": # puede ser cualquira de ["N1_Selec", "N2_Selec", "Selec_None"]
                        ...
                        if self.end_node:
                            ...
                            if all(field_name in self.line_mt_layer.fields().names() for field_name in field_names):
                                ...
                                if getattr(self, 'clonando', False) and getattr(self, 'cloned_element', None):
                                    original_feat = find_line_by_uuid(self.line_mt_layer, self.cloned_element)
                                    if original_feat:
                                        exclude_fields = ["id", "start_node", "end_node", "length", "units"]
                                        for field in self.line_mt_layer.fields():

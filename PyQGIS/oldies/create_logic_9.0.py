import os
import uuid
from PyQt5.QtGui import QIcon, QCursor, QPixmap, QFont, QColor
from PyQt5.QtWidgets import QToolBar, QToolButton, QButtonGroup, QAction, QActionGroup, QFileDialog, QComboBox, QDialog, QVBoxLayout, QDialogButtonBox, QMenu, QFormLayout, QLineEdit, QPushButton, QDialogButtonBox, QInputDialog
from qgis.utils import iface
from qgis.PyQt.QtCore import QSize, Qt
from qgis.PyQt.QtWidgets import QProgressBar, QApplication, QLabel
from qgis.core import Qgis, QgsProject, QgsVectorLayer, QgsField, QgsFeature, QgsGeometry, QgsPointXY, QgsPoint, QgsSingleSymbolRenderer, QgsLineSymbol, QgsRectangle, QgsGeometry, QgsPointXY, QgsPalLayerSettings, QgsTextFormat, QgsVectorLayerSimpleLabeling, QgsDistanceArea
...
class DSS():
    ...
    def resolver_DSS_hourly(self, hour, multiplicador_carga):
        # Format the hour for OpenDSS as HH.MM
        formatted_hour = f"{hour}.00"
        # Set the simulation time to the specified hour
        self.dssText.Command = f"Set Time={formatted_hour}"
        # Set the mode to daily to allow time-based simulation
        self.dssText.Command = "Set Mode=Daily"
        self.dssText.Command = "Set ControlMode=Static"
        # Multiply the nominal load values by the load multiplier
        self.dssSolution.LoadMult = multiplicador_carga
        # Solve the power flow for the specified hour
        self.dssSolution.Solve()
...
class CustomToolbarPanel:
    ...
    def init_panel(self):
        ...
        button_names = [
            ...
            ("PowerFlow", "Calcular flujo de potencia")
        ]
        ...
        def create_handler(button_name, cursor_icon_path):
            def handler():
                if button_name.lower() == "librerias":
                    ...
                    for comp_name, comp_label in components.items():
                        ...
                        # Agregar acciones Importar y Crear
                    ...
                    button = self.tool_buttons[-1]  # Get the last button (Librerias)
    ...
    def handle_library_action(self, component_type, action_type):
        """Maneja las acciones de importar o crear para los componentes de la librería"""
        if action_type == "import":
            ...
            elif component_type == "settings":
                file_path, _ = QFileDialog.getOpenFileName(None, "Seleccionar archivo Settings", "", "DSS Files (*.dss)")
                if file_path:
                    settings_layers = QgsProject.instance().mapLayersByName("settings")
                    if settings_layers:
                        importar_settings(file_path, settings_layers[0])
        else:  # action_type == "create"
            # Llamar a la función de creación correspondiente
            ...
            if output_dir:
                if component_type == "linecode":
                    ...
                    line_geometry_layers = QgsProject.instance().mapLayersByName("line_geometry")
                    if line_geometry_layers:
                        create_linegeometry_dss(line_geometry_layers[0], output_dir)
                elif component_type == "linespacing":
                    line_spacing_layers = QgsProject.instance().mapLayersByName("line_spacing")
                    if line_spacing_layers:
                        create_linespacing_dss(line_spacing_layers[0], output_dir)
                elif component_type == "wiredata":
                    wire_data_layers = QgsProject.instance().mapLayersByName("wire_data")
                    if wire_data_layers:
                    ...
                    settings_layers = QgsProject.instance().mapLayersByName("settings")
                    if settings_layers:
                        create_settings_dss(settings_layers[0], output_dir)
    ...
    def handle_button_click(self, button_name, cursor_icon_path):
        ...
        elif button_name.lower() == "powerflow":
...
class CustomMapTool(QgsMapTool):
                ...
                if estado_flujo_trabajo == "seleccion":
                    ...
                    if encontrados:
                        ...
                        if ok and seleccion:
                            ...
                            if tipo == "Carga":
                                ...
                                if accion_seleccionada == accion_eliminar:
                                    ...
                                    self.node_mt_layer.deleteFeature(feat.id())
                        ...
                        if self.selected_line:
                            ...
                            if accion_seleccionada == accion_editar:
                                self.show_edit_form_line(self.selected_line, self.line_mt_layer)
                            elif accion_seleccionada == accion_eliminar:
                                self.line_mt_layer.startEditing()
                                self.line_mt_layer.deleteFeature(self.selected_line.id())
                    ...
                    if estado_seleccion_linea == "Selec_None": # puede ser cualquira de ["N1_Selec", "N2_Selec", "Selec_None"]
                        ...
                        if self.end_node:
                            ...
                            if all(field_name in self.line_mt_layer.fields().names() for field_name in field_names):
                                ...
                                distance_calculator.setSourceCrs(self.line_mt_layer.crs(), QgsProject.instance().transformContext())
    ...
    def updateLine_mtGeometry(self, line_feat):
        ...
        self.line_mt_layer.changeAttributeValue(line_feat.id(), self.line_mt_layer.fields().indexFromName("length"), str(round(length_meters, 2)))



    def show_edit_form_generator(self, feature):
        print("Mostrando formulario de edición para Generador")

    def show_edit_form_pv(self, feature):
        print("Mostrando formulario de edición para Fotovoltaico")

    def show_edit_form_capacitor(self, feature):
        print("Mostrando formulario de edición para Capacitor")

    def show_edit_form_transformador(self, feature):
        print("Mostrando formulario de edición para Transformador")
        ...
        for field_name in element.fields().names():
            ...
            if field_name == "geometry":
                ...
                if field_value is None:
                    ...
                    combo.setCurrentIndex(combo.findText(str(field_value)))
                self.fields[field_name] = combo
                form_layout.addRow("geometry", combo)

            elif field_name == "linecode":
                combo = QComboBox()
                combo.addItem("NULL")
                if linecode_layer:
                ...
                if field_value is None:
                    ...
                    combo.setCurrentIndex(combo.findText(str(field_value)))
                self.fields[field_name] = combo
                form_layout.addRow("linecode", combo)
    ...
    for tipo, capa in capas_dict.items():
        ...
        if features:
            if tipo not in encontrados:
                encontrados[tipo] = []
            encontrados[tipo].extend(features)
...
def reset_power_flow():
    # Iniciar edición en la capa de nodos MT
    node_mt_layer.startEditing()
    for node in node_mt_layer.getFeatures():
        node.setAttribute("pu1", None)  # Restablecer el valor de pu1 a NULL
        node_mt_layer.updateFeature(node)
    node_mt_layer.commitChanges()

    # Iniciar edición en la capa de líneas MT
    line_mt_layer.startEditing()
    for line in line_mt_layer.getFeatures():
        line.setAttribute("P.Activa", None)  # Restablecer el valor de P.Activa a NULL
        line_mt_layer.updateFeature(line)
    line_mt_layer.commitChanges()

    # Iniciar edición en la capa de cargas
    load_layer.startEditing()
    for load in load_layer.getFeatures():
        load.setAttribute("P.Activa", None)  # Restablecer el valor de P.Activa a NULL
        load.setAttribute("P.Reactiva", None)  # Restablecer el valor de P.Reactiva a NULL
...
def power_flow(node_mt_layer, line_mt_layer, load_layer):
    # Paso 1: Exportar archivos DSS
    export_dss_fixed_location(node_mt_layer, line_mt_layer, transformer_layer, generator_layer, pv_system_layer, et_layer, load_layer, switch_layer, capacitor_layer)
    # Paso 2: Conectar a la red generada y realizar el cálculo de flujo de potencia

    # Reference the correct path for Master.dss
    #script_dir = os.getcwd()
    script_dir = project_path
    ...
    # if ok:
    #     # Call the resolver_DSS_hourly method with the user-provided hour
    ...
    for node in node_mt_layer.getFeatures():
        ...
        pu1 = dss_instance.dssBus.puVmagAngle[0]  # Obtener el valor de pu1
        pu1 = round(pu1, 5)  # Restringir a 5 dígitos decimales
    ...
    for line in line_mt_layer.getFeatures():
        ...
        #p_activa = abs(dss_instance.dssCktElement.TotalPowers[0])  # Obtener el valor de P.Activa
        p_activa = dss_instance.dssCktElement.TotalPowers[0]  # Obtener el valor de P.Activa
        p_activa = round(p_activa, 3)  # Restringir a 3 dígitos decimales
    ...
    for load in load_layer.getFeatures():
        ...
        p_activa = abs(dss_instance.dssCktElement.TotalPowers[0])  # Obtener el valor de P.Activa
        p_reactiva = abs(dss_instance.dssCktElement.TotalPowers[1])  # Obtener el valor de P.Reactiva
        p_activa = round(p_activa, 3)  # Restringir a 3 dígitos decimales
        p_reactiva = round(p_reactiva, 3)  # Restringir a 3 dígitos decimales
...
def export_dss_fixed_location(node_mt_layer, line_layer, transformer_layer, generator_layer, pv_system_layer, et_layer, load_layer, switch_layer, capacitor_layer):
    # Calculate the fixed directory path
    #script_dir = os.getcwd()
    script_dir = project_path
    ...
    master_dss_path = os.path.join(power_flow_dir, "Master.dss")
    ...
    create_vsource_dss(et_layer, power_flow_dir, progress, progress_level, increment_per_element)
    messageLabel.setText("Exportando líneas...")
    create_line_dss(line_layer, power_flow_dir, progress, progress_level, increment_per_element)
    messageLabel.setText("Exportando transformadores...")
    create_transformer_dss(transformer_layer, power_flow_dir, progress, progress_level, increment_per_element)
    messageLabel.setText("Exportando cargas...")
    create_load_dss(load_layer, power_flow_dir, progress, progress_level, increment_per_element)
    messageLabel.setText("Exportando interruptores...")
    create_switch_dss(switch_layer, power_flow_dir, progress, progress_level, increment_per_element)
    messageLabel.setText("Exportando capacitores...")
    create_capacitor_dss(capacitor_layer, power_flow_dir, progress, progress_level, increment_per_element)
    messageLabel.setText("Exportando sistemas fotovoltaicos...")
    create_pv_system_dss(pv_system_layer, power_flow_dir, progress, progress_level, increment_per_element)
    messageLabel.setText("Exportando generadores...")
    create_generator_dss(generator_layer, power_flow_dir, progress, progress_level, increment_per_element)
    messageLabel.setText("Exportando coordenadas de nodos...")
    create_buscoords_csv(node_mt_layer, power_flow_dir, progress, progress_level, increment_per_element)

    # Exportar librerías geométricas con mensaje y progreso

    messageLabel.setText("Exportando LineCode...")
    linecode_layers = QgsProject.instance().mapLayersByName("linecode")
    if linecode_layers:
        create_linecode_dss(linecode_layers[0], power_flow_dir, messageLabel, progress, progress_level, increment_per_element)
        progress_level += increment_per_element * max(1, linecode_layers[0].featureCount())
    else:
        print("No se encontró la capa 'linecode'")

    messageLabel.setText("Exportando LineGeometry...")
    linegeometry_layers = QgsProject.instance().mapLayersByName("linegeometry")
    if linegeometry_layers:
        create_linegeometry_dss(linegeometry_layers[0], power_flow_dir, messageLabel, progress, progress_level, increment_per_element)
        progress_level += increment_per_element * max(1, linegeometry_layers[0].featureCount())
    else:
        print("No se encontró la capa 'linegeometry'")

    messageLabel.setText("Exportando LineSpacing...")
    linespacing_layers = QgsProject.instance().mapLayersByName("linespacing")
    if linespacing_layers:
        create_linespacing_dss(linespacing_layers[0], power_flow_dir, messageLabel, progress, progress_level, increment_per_element)
        progress_level += increment_per_element * max(1, linespacing_layers[0].featureCount())
    else:
        print("No se encontró la capa 'linespacing'")

    messageLabel.setText("Exportando WireData...")
    wiredata_layers = QgsProject.instance().mapLayersByName("wiredata")
    if wiredata_layers:
        create_wiredata_dss(wiredata_layers[0], power_flow_dir, messageLabel, progress, progress_level, increment_per_element)
        progress_level += increment_per_element * max(1, wiredata_layers[0].featureCount())
    else:
        print("No se encontró la capa 'wiredata'")

    messageLabel.setText("Exportando LoadShape...")
    loadshape_layers = QgsProject.instance().mapLayersByName("loadshape")
    if loadshape_layers:
        loadshape_layer = loadshape_layers[0]
        create_loadshape_dss(loadshape_layer, power_flow_dir)
    else:
        print("No se encontró la capa 'loadshape'")

    messageLabel.setText("Exportando TShape...")
    tshape_layers = QgsProject.instance().mapLayersByName("tshape")
    if tshape_layers:
        tshape_layer = tshape_layers[0]
        create_tshape_dss(tshape_layer, power_flow_dir)
    else:
        print("No se encontró la capa 'tshape'")

    messageLabel.setText("Exportando XYcurve...")
    xycurve_layers = QgsProject.instance().mapLayersByName("xycurve")
    if xycurve_layers:
        xycurve_layer = xycurve_layers[0]
        create_xycurve_dss(xycurve_layer, power_flow_dir)
    else:
        print("No se encontró la capa 'xycurve'")
...
def export_dss(node_mt_layer, line_layer, transformer_layer, generator_layer, pv_system_layer, et_layer, load_layer, switch_layer, capacitor_layer):
    ...
    linecode_layers = QgsProject.instance().mapLayersByName("linecode")
    if linecode_layers:
        create_linecode_dss(linecode_layers[0], folder_path, messageLabel, progress, progress_level, increment_per_element)
        progress_level += increment_per_element * max(1, linecode_layers[0].featureCount())
    else:
        print("No se encontró la capa 'linecode'")
    ...
    linegeometry_layers = QgsProject.instance().mapLayersByName("linegeometry")
    if linegeometry_layers:
        create_linegeometry_dss(linegeometry_layers[0], folder_path, messageLabel, progress, progress_level, increment_per_element)
        progress_level += increment_per_element * max(1, linegeometry_layers[0].featureCount())
    else:
        print("No se encontró la capa 'linegeometry'")
    ...
    linespacing_layers = QgsProject.instance().mapLayersByName("linespacing")
    if linespacing_layers:
        create_linespacing_dss(linespacing_layers[0], folder_path, messageLabel, progress, progress_level, increment_per_element)
        progress_level += increment_per_element * max(1, linespacing_layers[0].featureCount())
    else:
        print("No se encontró la capa 'linespacing'")
    ...
    wiredata_layers = QgsProject.instance().mapLayersByName("wiredata")
    if wiredata_layers:
        create_wiredata_dss(wiredata_layers[0], folder_path, messageLabel, progress, progress_level, increment_per_element)
        progress_level += increment_per_element * max(1, wiredata_layers[0].featureCount())
    else:
        print("No se encontró la capa 'wiredata'")
    ...
    if loadshape_layers:
        loadshape_layer = loadshape_layers[0]
        create_loadshape_dss(loadshape_layer, folder_path)
    else:
        print("No se encontró la capa 'loadshape'")
    ...
    if tshape_layers:
        tshape_layer = tshape_layers[0]
        create_tshape_dss(tshape_layer, folder_path)
    else:
        print("No se encontró la capa 'tshape'")
    ...
    if xycurve_layers:
        xycurve_layer = xycurve_layers[0]
        create_xycurve_dss(xycurve_layer, folder_path)
    else:
        print("No se encontró la capa 'xycurve'")
...
def create_loadshape_dss(loadshape_layer, output_dir):
    ...
    with open(filepath, "w") as f:
        for feature in loadshape_layer.getFeatures():
            name = feature["id"]
            npts = feature["npts"]
            ...
            # if csvfile != None:
            #     line = f"New LoadShape.{name} npts={npts} interval={interval} csvfile={csvfile}\n"
            #     f.write(line)
            # else:
            #     line = f"New LoadShape.{name} npts={npts} interval={interval} mult={mult}\n"
            #     f.write(line)
    
            line = f"New LoadShape.{name} npts={npts} interval={interval} mult={mult}\n"
            f.write(line)

    print(f"Archivo LoadShape.dss generado en: {filepath}")
...
def create_tshape_dss(tshape_layer, output_dir):
    ...
    print(f"Archivo TShape.dss generado en: {filepath}")
...
def create_xycurve_dss(xycurve_layer, output_dir):
    ...
    print(f"Archivo XYcurve.dss generado en: {filepath}")

def create_linecode_dss(linecode_layer, folder_path, messageLabel=None, progress=None, progress_level=0, increment=1):
    ...
    with open(output_path, 'w') as f:
        if linecode_layer.featureCount() == 0:
            ...
            for feat in linecode_layer.getFeatures():
                ...
                if progress:
                    progress_level += increment
                    progress.setValue(int(progress_level))
                    QApplication.processEvents()


def create_linegeometry_dss(linegeometry_layer, folder_path, messageLabel=None, progress=None, progress_level=0, increment=1):
    ...
    with open(output_path, 'w') as f:
        if linegeometry_layer.featureCount() == 0:
            ...
            for feat in linegeometry_layer.getFeatures():
                ...
                wires = feat["wires"]
                f.write(f"New LineGeometry.{name} nconds={nconds} nphases={nphases} spacing={spacing} wires={wires}\n")
                if progress:
                    progress_level += increment
                    progress.setValue(int(progress_level))
                    QApplication.processEvents()

def create_linespacing_dss(linespacing_layer, folder_path, messageLabel=None, progress=None, progress_level=0, increment=1):
...
def create_wiredata_dss(wiredata_layer, folder_path, messageLabel=None, progress=None, progress_level=0, increment=1):
    ...
    with open(line_dss_path, 'w') as file:
        for feature in line_layer.getFeatures():
            ...
            if linecode and linecode.strip().lower() != "null":
                file.write(f"New Line.{id} bus1={start_node} bus2={end_node} length={length} units=m enabled={enabled} phases={phases} linecode={linecode}\n")
            elif geometry and geometry.strip().lower() != "null":
                file.write(f"New Line.{id} bus1={start_node} bus2={end_node} length={length} Geometry={geometry} Units=m\n")
            else:
                file.write(f"New Line.{id} bus1={start_node} bus2={end_node} length={length} units=m r1={r1} r0={r0} x1={x1} x0={x0} c1={c1} c0={c0} enabled={enabled} phases={phases}\n")
...
def create_pv_system_dss(pv_system_layer, folder_path, progress, progress_level, increment_per_element):
    ...
    with open(pv_system_dss_path, 'w') as file:
        for feature in pv_system_layer.getFeatures():
            ...
            file.write(f"New PVSystem.{id} phases={phases} bus1={start_node} kv={kv} irradiance={irradiance} pmpp={pmpp} temperature={temperature} kva={kva} effcurve={effcurve} daily={daily} tdaily={tdaily}\n")
            progress_level += increment_per_element
            progress.setValue(int(progress_level))
            QApplication.processEvents()
...
def reduce_network():
    ...
    special_layers = {
        ...
        seta_layer: ["wdg1_node", "wdg2_node"]
    }
    ...
    for node_id, lines in node_connections.items():
        ...
        start_node_feature = find_node_by_uuid(node_mt_layer, start_node)
        end_node_feature = find_node_by_uuid(node_mt_layer, end_node)
        ...
        if start_node_feature and end_node_feature:
            ...
            new_line = QgsFeature(line_mt_layer.fields())
            ...
            line_mt_layer.addFeature(new_line)
            line_mt_layer.deleteFeature(line1.id())
            line_mt_layer.deleteFeature(line2.id())
            
            # Remove the node
            node_mt_layer.deleteFeature(find_node_by_uuid(node_mt_layer, node_id).id())
            nodes_eliminated += 1
        else:

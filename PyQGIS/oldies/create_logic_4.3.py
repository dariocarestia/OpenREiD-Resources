...
class CustomMapTool(QgsMapTool):
    ...
    def canvasMoveEvent(self, event):
        ...
        #-------------EDITANDO-------------------------------------------------
            ...
            nearby_line, nearby_node = find_nearest_line_index(self.line_layer, self.node_layer, QgsPointXY(event.mapPoint()))
            
            if nearby_line:
                print(f"Línea cercana: {nearby_line.id()}, Nodo cercano: {nearby_node.id()}")

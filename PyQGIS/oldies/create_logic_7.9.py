...
#def import_dss(node_mt_layer, node_bt_layer, line_mt_layer, line_bt_layer, generator_layer, pv_system_layer, load_layer, switch_layer, transformer_layer, et_layer, seta_layer, capacitor_layer):
    ...
    # Importar LineGeometry.dss
    line_geometry_file = os.path.join(os.path.dirname(dss_path), "LineGeometry.dss")
    if os.path.exists(line_geometry_file):
        # Obtener la capa de geometrías de línea existente
        line_geometry_layers = QgsProject.instance().mapLayersByName("linegeometry")
        if not line_geometry_layers:
            print("Error: No se encontró la capa de geometrías de línea")
            return
        
        line_geometry_layer = line_geometry_layers[0]
        ...
        with open(line_geometry_file, 'r') as f:
            for line in f:
                line = line.strip()
                ...
                # Procesar línea de LineGeometry
                if line.startswith('New LineGeometry.'):
                    ...
                    while i < len(parts):
                        if '=' in parts[i]:
                            ...
                            # Si el valor comienza con paréntesis, buscar el paréntesis de cierre
                            if value.startswith('('):
                                # Reconstruir el valor completo
                                full_value = value
                                i += 1
                    ...
                    # Crear feature para la capa de geometrías de línea
                    line_geometry_feature = QgsFeature()
                    line_geometry_feature.setAttributes([
                        name,  # nombre
                        params.get('nconds', ''),  # nconds
                        params.get('nphases', ''),  # nphases
                        params.get('spacing', ''),  # spacing
                        params.get('wires', '')  # wires
                    ])
                    
                    # Agregar feature a la capa
    ...
    # Importar LineSpacing.dss
    line_spacing_file = os.path.join(os.path.dirname(dss_path), "LineSpacing.dss")
    if os.path.exists(line_spacing_file):
        # Obtener la capa de espaciados de línea existente
        line_spacing_layers = QgsProject.instance().mapLayersByName("linespacing")
        if not line_spacing_layers:
            print("Error: No se encontró la capa de espaciados de línea")
            return
        
        line_spacing_layer = line_spacing_layers[0]
        ...
        with open(line_spacing_file, 'r') as f:
            for line in f:
                line = line.strip()
                # Ignorar líneas vacías y comentarios
                if not line or line.startswith('//'):
                ...
                if line.startswith('New LineSpacing.'):
                    ...
                    # Crear feature para la capa de espaciados de línea
                    line_spacing_feature = QgsFeature()
                    line_spacing_feature.setAttributes([
                        name,  # id
                        params.get('nconds', ''),  # nconds
                        params.get('nphases', ''),  # nphases
                        params.get('units', ''),  # units
                        params.get('x', ''),  # x
                        params.get('h', '')  # h
                    ])
    ...
    # Importar WireData.dss
    wire_data_file = os.path.join(os.path.dirname(dss_path), "WireData.dss")
    if os.path.exists(wire_data_file):
        # Obtener la capa de datos de conductores existente
        wire_data_layers = QgsProject.instance().mapLayersByName("wiredata")
        if not wire_data_layers:
            print("Error: No se encontró la capa de datos de conductores")
            return
        
        wire_data_layer = wire_data_layers[0]
        ...
        with open(wire_data_file, 'r') as f:
            for line in f:
                line = line.strip()
                # Ignorar líneas vacías y comentarios
                if not line or line.startswith('//'):
                ...
                if line.startswith('New WireData.'):
                    ...
                    # Crear feature para la capa de datos de conductores
                    wire_data_feature = QgsFeature()
                    wire_data_feature.setAttributes([
                        name,  # nombre
                        params.get('Rdc', ''),  # Rdc
                        params.get('Runits', ''),  # Runits
                        params.get('diam', ''),  # diam
                        params.get('Radunits', ''),  # Radunits
                        params.get('emergamps', '')  # emergamps
                    ])
...
def create_line_dss(line_layer, folder_path, progress, progress_level, increment_per_element):
    ...
    with open(line_dss_path, 'w') as file:
        for feature in line_layer.getFeatures():
            ...
            rmatrix = feature["Rmatrix"]
            xmatrix = feature["Xmatrix"]
            cmatrix = feature["Cmatrix"]
            ...
            #file.write(f"New Line.{id} bus1={start_node} bus2={end_node} length={length} units=m Rmatrix={rmatrix} Xmatrix={xmatrix} Cmatrix={cmatrix} enabled={enabled} phases={phases}\n")

...
#def import_dss(node_mt_layer, node_bt_layer, line_mt_layer, line_bt_layer, generator_layer, pv_system_layer, load_layer, switch_layer, transformer_layer, et_layer, seta_layer, capacitor_layer):
    ...
    if lines.First != 0:
        while True:
            ...
            bus1_feature = find_node_by_uuid(node_mt_layer, bus1_base)
            bus2_feature = find_node_by_uuid(node_mt_layer, bus2_base)
    ...
    if transformers.First != 0:
        while True:
            ...
            if bus1_feature:
                ...
                transformer_feature.setAttribute("xfrmcode", str(transformers.XfmrCode))
...
def importar_xfmrcode(file_path, xfmrcode_layer):
    ...
    if not os.path.exists(file_path):
        print("Archivo XfrmCode.dss no encontrado")
        return
...
def export_dss_location(export_dir, node_mt_layer, line_layer, transformer_layer, generator_layer, pv_system_layer, et_layer, load_layer, switch_layer, capacitor_layer):
    ...
    messageLabel.setText("Exportando XfmrCode...")
    xfmrcode_layers = QgsProject.instance().mapLayersByName("xfmrcode")
    if xfmrcode_layers:
    ...
    with open(line_dss_path, 'w') as file:
        for feature in line_layer.getFeatures():
            ...
            if linecode and linecode.strip().lower() != "null":
                file.write(f"New Line.{id} bus1={bus1} bus2={bus2} length={length} units=m enabled={enabled} phases={phases} linecode={linecode}\n")
                ...
                file.write(f"New Line.{id} bus1={bus1} bus2={bus2} length={length} units=m r1={r1} r0={r0} x1={x1} x0={x0} c1={c1} c0={c0} enabled={enabled} phases={phases}\n")
            progress_level += increment_per_element
            progress.setValue(int(progress_level))
            QApplication.processEvents()

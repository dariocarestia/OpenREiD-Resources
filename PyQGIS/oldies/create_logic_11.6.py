...
def power_flow(node_layer, line_layer, load_layer):
    ...
    for node in node_layer.getFeatures():
        ...
        voltages_LL = round(voltages_LL, 3)  # Restringir a 5 dígitos decimales
    ...
    for line in line_layer.getFeatures():
        ...
        p_reactiva = dss_instance.dssCktElement.TotalPowers[1]  # Obtener el valor de P.Activa
...
def export_dss_location(export_dir, node_layer, line_layer, transformer_layer, generator_layer, pv_system_layer, et_layer, load_layer, switch_layer, capacitor_layer):
    ...
    total_elements = node_layer.featureCount() + line_layer.featureCount() + transformer_layer.featureCount() + et_layer.featureCount() + load_layer.featureCount() + switch_layer.featureCount()

    total_elements = (node_layer.featureCount() + line_layer.featureCount() + transformer_layer.featureCount() +
    ...
    create_buscoords_csv(node_layer, export_dir, progress, progress_level, increment_per_element)
...
def create_buscoords_csv(node_layer, folder_path, progress, progress_level, increment_per_element):

...
class CustomMapTool(QgsMapTool):
    ...
    def canvasPressEvent(self, event):
        ...
        if event.button() == Qt.LeftButton:
            ...
            #-------------EDITANDO-------------------------------------------------
            # nearby_line, nearby_node, point_on_line = find_nearest_line_index(self.line_layer, self.node_layer, QgsPointXY(event.mapPoint()))
...
#         closest_node = start_node if start_node_distance < end_node_distance else end_node

#         return nearest_line, closest_node
...
def find_nearest_line_index(line_layer, node_layer, target_point):    
    ...
    if selected_lines:
        ...
        if start_node_distance < end_node_distance:
            ...
            terminal = "1"
            distance_along_line = line_length * 0.2

        else:
            closest_node = end_node
            terminal = "2"
            distance_along_line = line_length * 0.8

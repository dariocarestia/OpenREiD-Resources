...
from PyQt5.QtWidgets import QProgressBar, QToolBar, QToolButton, QButtonGroup, QAction, QActionGroup
from qgis.utils import iface
from qgis.PyQt.QtCore import QSize, Qt, QCoreApplication
from qgis.core import QgsProject, QgsVectorLayer, QgsField, QgsFeature, QgsGeometry, QgsPointXY, QgsPoint, QgsSingleSymbolRenderer, QgsLineSymbol
...
class CustomToolbarPanel:
    ...
    def init_panel(self):
        ...
        # Agregar la barra de progreso a la caja de herramientas
        self.toolbox.addWidget(self.progress_bar)
    ...
    def handle_button_click(self, button_name, cursor_icon_path):
        ...
        if button_name.lower() == "seleccion":
            ...
            estado_edicion = "Selec_None"
            iface.mapCanvas().unsetCursor()
            progress_bar = QProgressBar()  # Crear una nueva barra de progreso
            progress_bar.setRange(0, 100)  # Establecer el rango de la barra de progreso
            self.progress_bar = progress_bar  # Asignar la barra de progreso a self.progress_bar
            self.progress_bar.setValue(0)  # Resetear la barra de progreso
            import_dss(self.map_tool.node_mt_layer, self.map_tool.line_mt_layer, self.progress_bar)  # Pasar la barra de progreso
        else:
            ...
            if event.button() == Qt.RightButton:
                ...
                if estado_flujo_trabajo == "seleccion":
                    ...
                    if estado_seleccion_linea == "Selec_None": # puede ser cualquira de ["N1_Selec", "N2_Selec", "Selec_None"]
                        ...
                        self.end_node = find_nearest_node_index(self.node_mt_layer, QgsPointXY(event.mapPoint()))
                        if self.end_node:
                            #estado_seleccion_linea = "Selec_None"                                                
...
def import_dss1(node_mt_layer, line_layer):

    # Obtener la ruta del directorio de trabajo actual
    script_dir = os.getcwd()
    dss_path = os.path.join(script_dir, "dss" , "Master_Caso_Base.dss")

    # Verificar si el archivo DSS existe
    if not os.path.exists(dss_path):
        print(f"Archivo DSS no encontrado: {dss_path}")
        return

    objeto = DSS(dss_path)

    print("Versión de Opendss:", objeto.version_DSS())

    # Resolver el Flujo de Potencia
    objeto.compilar_DSS()
    objeto.resolver_DSS_snapshot(1.0)

    # Nombre y tamaño de todas las líneas
    nombre_lineas, tamaño_lineas = objeto.obtener_nombre_y_tamaño_lineas()
    print("Nombres de las líneas:", str(nombre_lineas))
    print("Tamaños de las líneas:", str(tamaño_lineas))


def import_dss(node_mt_layer, line_layer, progress_bar):
    # Obtener la ruta del directorio de trabajo actual
    script_dir = os.getcwd()
    dss_path = os.path.join(script_dir, "dss", "Master_Caso_Base.dss")
    ...
    # Agregar líneas a la capa de líneas
    line_layer.startEditing()
    if lines.First != 0:  # Mover al primer elemento, verificar si hay líneas
        while True:
            ...
            if bus1_feature is None or bus2_feature is None:
                print(f"Error: Nodo {bus1} o {bus2} no encontrado.")
                if lines.Next == 0:
                    break
                continue
            ...
            if lines.Next == 0:  # Mover al siguiente elemento, salir si no hay más
                break
    line_layer.commitChanges()

    print("Importación desde OpenDSS completada.")
    progress_bar.setValue(total_steps)  # Asegurarse de que la barra de progreso esté completa



# def import_dss(node_mt_layer, line_layer):
#     # Obtener la ruta del directorio de trabajo actual
#     script_dir = os.getcwd()
#     dss_path = os.path.join(script_dir, "dss", "Master_Caso_Base.dss")

...
ubicacion_switch = 0.9 # Determina la ubicación en porcentaje de la longitud de línea del switch correspondiente al terminal 2


class CustomToolbarPanel:
    def __init__(self, map_tool, node_layer, line_layer, generator_layer, load_layer, switch_layer, transformer_layer):
        self.toolbox = None
        self.tool_buttons = []
    ...
    def init_panel(self):
        ...
        button_names = ["Seleccion", "Nodo", "Linea", "Interruptor", "Carga", "Generador", "Transformador"]
...
class CustomMapTool(QgsMapTool):
    def __init__(self, canvas, node_layer, line_layer, generator_layer, load_layer, switch_layer, transformer_layer):
                ...
                if estado_seleccion_transformador == "Selec_None":
                    ...
                    if self.start_node:
                        ...
                        if all(field_name in self.transformer_layer.fields().names() for field_name in field_names):
                            ...
                            transformer_feature.setAttribute("wdg2_node", self.start_node.attribute("id"))
...
custom_map_tool = CustomMapTool(iface.mapCanvas(), node_layer, line_layer, generator_layer, load_layer, switch_layer, transformer_layer)
iface.mapCanvas().setMapTool(custom_map_tool)

# Crear una instancia de la clase y mostrar la caja de herramientas
custom_toolbar_panel = CustomToolbarPanel(custom_map_tool, node_layer, line_layer, generator_layer, load_layer, switch_layer, transformer_layer)

...
class CustomToolbarPanel:
    def __init__(self, map_tool, node_layer, line_layer, generator_layer):
        self.toolbox = None
        self.tool_buttons = []
        ...
        self.generator_layer = line_layer  # Referencia a la capa de lineas
...
class CustomMapTool(QgsMapTool):
    def __init__(self, canvas, node_layer, line_layer, generator_layer):
    ...
    def canvasMoveEvent(self, event):
        ...
        elif estado_flujo_trabajo == "seleccion":

            if estado_seleccion_nodo == "N_Selec":
        ...
        if estado_edicion == "Moviendo_Nodo" and self.selected_node:
            ...
            self.node_layer.triggerRepaint()
            #[Aqui realizar el movimiento objetos asociados]
            redraw_associated_objects(self.selected_node.attribute("id"), self.node_layer, self.line_layer, self.generator_layer, QgsPointXY(clicked_point))
    ...
    def canvasReleaseEvent(self, event):
        ...
        if estado_edicion == "Moviendo_Nodo":
            ...
            estado_edicion = "Selec_None"
            self.selected_node = None
            print("Movimiento del nodo finalizado")
...
custom_map_tool = CustomMapTool(iface.mapCanvas(), node_layer, line_layer, generator_layer)
iface.mapCanvas().setMapTool(custom_map_tool)

# Crear una instancia de la clase y mostrar la caja de herramientas
custom_toolbar_panel = CustomToolbarPanel(custom_map_tool, node_layer, line_layer, generator_layer)

...
class CustomMapTool(QgsMapTool):
    def __init__(self, canvas, node_layer, line_layer):
        ...
        self.temp_line_layer_provider.addAttributes([QgsField("dummy", QVariant.String)])  # Agregar un campo dummy para evitar KeyError
    ...
    def canvasMoveEvent(self, event):
        ...
        if estado_flujo_trabajo == "linea":
            ...
            if estado_seleccion_linea == "N1_Selec":
                # Limpiar la capa temporal antes de añadir una nueva línea auxiliar
                self.temp_line_layer.startEditing()
                ...
                temp_line_feature.setAttribute("dummy", "")  # Añadir valor dummy al campo dummy
                ...
                self.temp_line_layer_provider.addFeatures([temp_line_feature])
                self.temp_line_layer.commitChanges()

                # Guardar la referencia a la nueva línea auxiliar
                self.temp_line_feature = temp_line_feature

                # Actualizar visualmente la capa temporal
                self.temp_line_layer.triggerRepaint()

                # Forzar la actualización del lienzo
                self.canvas.refreshAllLayers()

            elif estado_seleccion_linea == "Selec_None":
                # Eliminar la capa temporal y reiniciar la referencia a la línea auxiliar
                if self.temp_line_feature:
                    self.temp_line_layer_provider.deleteFeatures([self.temp_line_feature.id()])
                    self.temp_line_layer.commitChanges()
                    self.temp_line_feature = None

    ...
    def canvasPressEvent(self, event):
        ...
        if event.button() == Qt.LeftButton:
            ...
            if estado_flujo_trabajo == "selección":
                ...
                self.node_feature = QgsFeature()

                # Imprimir la lista de campos disponibles en la capa de nodos
                print("Campos disponibles en la capa de nodos:", self.node_layer.fields().names())
                ...
                if field_name in self.node_layer.fields().names():
                    print("Campos disponibles en la capa de nodos:", self.node_layer.fields().names())
                    self.node_feature.setAttribute(field_name, unique_node_id)
                    self.node_feature.setGeometry(QgsGeometry.fromPointXY(QgsPointXY(event.mapPoint())))
                ...
                nearest_node = find_nearest_node(self.node_layer, event.mapPoint())

                if nearest_node:
                    print(f"Nodo más cercano ID: {nearest_node.id()}")

                    # Aquí deberías agregar la lógica para obtener las coordenadas del nodo
                    # y luego construir y agregar la línea.
                    start_point = QgsPointXY(nearest_node.geometry().asPoint())

                    # Agregar la lógica para el segundo clic y la creación de la línea
                    # ...

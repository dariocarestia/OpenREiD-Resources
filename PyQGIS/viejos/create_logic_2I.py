    ...
    def canvasMoveEvent(self, event):
        ...
        if estado_flujo_trabajo == "linea":
            ...
            if estado_seleccion_linea == "N1_Selec":
                ...
                # Crear una línea entre self.start_node y la posición actual del cursor
                line_points = [self.start_node.geometry().asPoint(), current_point]

                # Dibujar la línea auxiliar en el lienzo
                self.draw_line_on_canvas(line_points)


    def draw_line_on_canvas(self, line_points):
        # Crear un objeto QPainter para dibujar en el canvas
        painter = QPainter(self.canvas.viewport())
        # Configurar el pincel y el color
        painter.setPen(QPen(Qt.red, 2, Qt.SolidLine))

        # Convertir los puntos a coordenadas de la pantalla
        screen_points = [self.canvas.mapToPixel(point) for point in line_points]
...
def find_nearest_node_index(node_layer, spatial_index, target_point):
    ...
    # Definir la distancia máxima en metros
    buffer_meters = 10  # Puedes ajustar este valor según tus necesidades

    scale = iface.mapCanvas().scale()
    #Ajusta max_distance_meters basado en la escala actual
    #Puedes ajustar el factor multiplicativo según tus necesidades
    buffer_meters = scale / 100  # Ejemplo: 1 metro por cada 1000 unidades en la escala
